...
bool AppController::exportCsv(const QUrl &fileUrl)
{
    const QString localFile = fileUrl.toLocalFile();
    if (localFile.isEmpty()) {
        setStatus(QStringLiteral("Export failed: no file selected"));
        return false;
    }

    QFile out(localFile);
    if (!out.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text)) {
        setStatus(QStringLiteral("Export failed: cannot write %1").arg(localFile));
        return false;
    }
    ...
    out.close();
    setStatus(QStringLiteral("Exported CSV: %1 (%2 rows)").arg(fileUrl.fileName()).arg(rows));
    return true;
}
